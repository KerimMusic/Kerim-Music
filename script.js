...
/* 🔥 DETECTOR DE KODULAR Y COMUNICACIÓN */
function isInKodular() {
    return typeof window.Kodular !== 'undefined' &&
           typeof window.Kodular.setWebViewString === 'function';
}
function sendToKodular(msg) {
    if (isInKodular()) {
        try { window.Kodular.setWebViewString(msg); } catch (_) {}
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    ['.menu-btn', '.heart-search-btn', '.share-btn', '.close-submenu',
     '.submenu-link', '.play-button'].forEach(sel => {
        document.querySelectorAll(sel).forEach(el => attachRipple(el));
    });
    ...
    /* 🔥 FUNCIÓN CLAVE: cargar una canción */
    function loadItem(item, autoplay = true) {
        ...
        // Siempre actualizamos el src del audio HTML (para el modo web)
        audioPlayer.src = src;
        ...
        // 🔥 SI ESTAMOS EN KODULAR: enviamos la URL al reproductor nativo
        if (isInKodular()) {
            sendToKodular("PLAY_URL:" + src);
        }

        // Reproducimos el audio HTML SOLO si NO estamos en Kodular
        if (autoplay && !isInKodular()) {
    ...
    playButton.addEventListener('click', () => {
        ...
        // Si estamos en Kodular, pedimos al reproductor nativo que haga toggle
        if (isInKodular()) {
            sendToKodular("TOGGLE_PLAY");
            return;
        }
        // Modo web normal
    ...
    const coverObserver = new MutationObserver(mutations => {
        mutations.forEach(mutation => {
            ...
            if (!item) return;
            if (item === currentItem) playerCover.src = img.getAttribute('src') || '';
        });
    });
    ...
    if (heartSearchBtn && searchContainer && searchInput) {
        ...
        searchInput.addEventListener('input', (e) => {
            ...
            document.querySelectorAll('.playlist-item').forEach(item => {
                ...
                item.style.display = (title.includes(query) || subtitle.includes(query)) ? 'flex' : 'none';
            });
        });
    }
        ...
        shareBtn.addEventListener('click', () => {
            ...
            if (navigator.share) {
                ...
                navigator.clipboard.writeText(txt).then(() => alert('¡Enlace copiado!'))
                    .catch(() => alert('Copia este enlace: ' + SHARE_URL));
            }
        });
    }
    ...
    function updateLikeUI() {
        ...
        const data = getLikeDataByKey(key);
        fsLikeBtn.classList.toggle('active', !!(data && data.liked !== false));
    }
    ...
    fsLikeBtn.addEventListener('click', async () => {
        ...
        if (isLikeLockedByKey(key)) {
            showNotification('No puedes manipular el botón de Me gusta durante 30 días.');
            return;
        }
        const data = getLikeDataByKey(key);
    ...
    function attachGesture(el, onGesture) {
        el.addEventListener('pointerdown', (e) => {
            ...
            const target = e.target, pid = e.pointerId;
            function onUp(e2) {
    ...
    attachGesture(player, ({ dx, dy, dt, target }) => {
        ...
        const onButton = target && target.closest && target.closest('button, #progress-bar, #play-button');
        if (!onProgress && !onButton && dt < 400 && absX < 12 && absY < 12) { openFullscreen(); return; }
        if (dt > 800) return;
    ...
    (function initFsTitleArtistLink() {
        ...
        fsTitleEl.addEventListener('pointerdown', () => { fsTitleEl.style.opacity = '0.7'; });
        ['pointerup', 'pointercancel', 'pointerleave'].forEach(evt => {
            fsTitleEl.addEventListener(evt, () => { fsTitleEl.style.opacity = ''; });
        });
    })();
    ...
    // 🔥 FUNCIONES GLOBALES PARA KODULAR
    window.kodularPlayPause = function() {
        if (playButton) playButton.click();
    };
    window.kodularNext = function() { goNextItem(); };
    window.kodularPrev = function() { goPrevItem(); };
...
(function () {
    ...
    function init() {
        ...
        if (typeof ADS === 'undefined' || !Array.isArray(ADS) || ADS.length === 0) return;
        var BEATS_PER_AD = 6;
        var SKIP_DELAY   = 5;
        var beatPlayCount = 0, lastSrc = '', isAdPlaying = false, adIndex = 0;
        var adOnComplete = null, currentAdMedia = null, countdownInterval = null, adTimeout = null;
        var originalPlay = audioPlayer.play.bind(audioPlayer);

        var overlay = document.createElement('div');
        overlay.id = 'ad-overlay';
        overlay.setAttribute('aria-hidden', 'true');
        overlay.innerHTML = '<div class="ad-inner"><div class="ad-label">ANUNCIO</div><div class="ad-media"></div><button class="ad-skip" type="button" disabled>Saltar anuncio (<span class="ad-countdown">' + SKIP_DELAY + '</span>)</button></div>';
...
(function () {
    ...
    function initArtistProfile() {
        ...
        let currentArtist = null, refreshTimer = null, gridObserver = null;
        const COLLAB_SPLIT = /\s+(?:ft\.?|feat\.?|featuring|con|&)\s+/i;
        ...
        function computeTotalPlays(items) {
            ...
            items.forEach(item => {
                const title = item.querySelector('.item-title')?.textContent.trim() || '';
                const doc = findFirebaseDoc(title);
                if (doc && typeof doc.data.reproducciones === 'number') total += doc.data.reproducciones;
            });
            return total;
        }
        ...
        function updatePlayingCard() {
            ...
            apGrid.querySelectorAll('.ap-card').forEach(card => {
                card.classList.toggle('playing', activeTitle && card.dataset.title === activeTitle);
            });
        }
        ...
        function renderProfile(artistName) {
            ...
            items.forEach(item => {
                ...
                if (cover) { const img = document.createElement('img'); img.src = cover; img.alt = title; img.loading = 'lazy'; card.appendChild(img); }
                const t = document.createElement('span');
                t.className = 'ap-card-title';
                t.textContent = title;
                card.appendChild(t);
                card.addEventListener('click', () => { activateArtistMode(artistName); item.click(); setTimeout(updatePlayingCard, 60); });
            ...
            refreshTimer = setInterval(() => {
                ...
                apTotal.textContent = formatNumber(computeTotalPlays(getArtistItems(currentArtist)));
            }, 1500);
...
(function () {
    ...
    const ICON_REPEAT = `<svg viewBox="0 0 24 24" aria-hidden="true"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/><text class="fs-repeat-one-mark" x="12" y="15.3" text-anchor="middle">1</text></svg>`;
    const ICON_SHUFFLE = `<svg viewBox="0 0 24 24" aria-hidden="true"><polyline points="16 3 21 3 21 8"/><line x1="4" y1="20" x2="21" y2="3"/><polyline points="21 16 21 21 16 21"/><line x1="15" y1="15" x2="21" y2="21"/><line x1="4" y1="4" x2="9" y2="9"/></svg>`;
...
(function () {
    'use strict';
    let observer = null, reorganizing = false, bootRetries = 0;
    function getItemTitle(item) {
        if (!item) return '';
        return item.querySelector('.item-title')?.textContent.trim() || '';
    }
    ...
    function reorganizeGrid() {
        ...
        const albums = new Map(), standalones = [];
    ...
    function boot() {
        ...
        (function loop() {
            ...
            if (hasItems && (fbReady || tries >= 20)) {
                buildAll(); initHistoryTracking(); initSearchToggle();
                return;
            }
            if (tries >= 40) return;
            setTimeout(loop, 200);
...
(function () {
    'use strict';
    const COLLAB_SPLIT = /\s+(?:ft\.?|feat\.?|featuring|con|&)\s+/i;
    let albumItems = [], albumQueue = [], albumIndex = 0, albumPlaying = false;
    let listMode = false, internalClick = false;
    ...
    function renderCarousel() {
        ...
        albumItems.forEach(item => {
            const title = getItemTitle(item), cover = getItemCover(item);
            const artist = getItemArtists(item)[0] || '';
            const card = document.createElement('button');
            card.type = 'button';
    ...
    function updateAlbumHighlight() {
        ...
        if (avCarousel) avCarousel.querySelectorAll('.av-card').forEach((card, i) => {
            card.classList.toggle('playing', albumItems[i] === active);
        });
        if (avList) avList.querySelectorAll('.av-row').forEach((row, i) => {
    ...
    function openAlbum(albumName, artistName) {
        ...
        renderCarousel(); renderList(); setListMode(false);
        if (albumView) {
    ...
    function initPlaylistObserver() {
        ...
        }).observe(pl, { subtree: true, attributes: true, attributeFilter: ['class'] });
    }
    function boot() {
...
(function () {
    ...
    function hideAllSongs() { getPlaylistItems().forEach(it => { it.style.display = 'none'; }); }
    function revealAllSongs() { getPlaylistItems().forEach(it => { it.style.display = 'flex'; }); }
    function init() {
        ...
        if (searchInput) {
            searchInput.addEventListener('input', (e) => {
                ...
                if (!q && !showAllSongs) hideAllSongs();
            });
        }
    }
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', init);
